...
//                produce the random # of clicks/sqeezes for next stae
                lemonSize = tree.pick()
//                reset squeeze count for squeeze state
                squeezeCount = 0

            }
            SQUEEZE -> {
//                increment squeeze count and decrement lemon size variable per click
                squeezeCount += 1
                lemonSize -= 1
//                move to next state when lemon side is zero


                lemonadeState = if (lemonSize == 0) {
                    DRINK
//                    if not zero, squeeze again
                } else {
                    SQUEEZE
                }
            }
            DRINK -> {
//                Reset state back to beginning and reset lemon size
...
//        set elements to reflect the current state
        setViewElements()
